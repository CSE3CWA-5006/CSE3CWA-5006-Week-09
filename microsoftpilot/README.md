...
This project reads its Microsoft Entra settings **only** from `appsettings.json`
(and `appsettings.<Environment>.json`) inside the project folder.

`Program.cs` deliberately removes the other configuration providers that
ASP.NET Core normally layers on top:

- User Secrets (`dotnet user-secrets`)
- Environment variables (e.g. `AzureAd__ClientId`)
- Command-line arguments

That means you cannot accidentally run this app with someone else's
credentials, and setting an environment variable will have no effect. Register
your own app in Microsoft Entra and fill in:
...
If `ClientId` is still `YOUR_CLIENT_ID`, the app stops at startup with a clear
message instead of failing later with an `AADSTS700016` error.

Do not commit a real `ClientSecret` to source control.
